...
默认使用 FP16 accumulator。FP32 模式使用 FP16 Tensor Core 输入、FP32
accumulator 与 FP32 epilogue compute，结果 D 仍写为 FP16。
...
两种累加器分别写入 `optimal_tuning/fp16` 和 `optimal_tuning/fp32`，
...
本测试用于估算推理性能时采用以下口径：Batch=1；Q/K/V 与 Up/Gate 不融合；Attention QK^T/PV 使用普通 GEMM 作为近似下界；A/B/C 和累加均为 FP16，用作 BF16 性能代理。该结果不包含 embedding、RMSNorm、RoPE、softmax/mask、激活、残差、KV 管理、kernel 间隙和多卡通信，因此不能当作完整端到端延迟。
