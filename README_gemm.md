...
脚本先生成算子形状，再按 `M/N/K` 去重：与 `L` 无关的 Linear GEMM 只运行一次；只有 `Attention QK^T` 和 `Attention PV` 会随 `L` 生成新形状。相同形状的来源算子通常合并记录，例如 `Q / Attention Out` 和 `K / V`。`MLP Up` 与 `MLP Gate` 是两个独立投影，均使用 `N=I`，即使形状相同也分别执行。

默认 Qwen2.5-7B 最终运行 15 个 Decode GEMM 用例。除 `MLP Up` 与 `MLP Gate` 分别执行外，其余相同形状会去重。每个 GEMM 用例都会比较：
...
Prefill 中所有算子的形状都会随 `S` 变化。Qwen2.5-7B 包含 71 个 GEMM 用例；同一 `S` 下的 `Q / Attention Out`、`K / V` 以及相同形状的 Attention 算子会合并记录，但 `MLP Up` 与 `MLP Gate` 分别执行。
...
- 阶段、相关 L/S 和来源算子名称（相同 `M/N/K` 通常合并，MLP Up 与 MLP Gate 除外）
- `M/N/K`
...
Excel 模板在原有 sheet 中预置了 Qwen2.5-7B 的 15 个 Decode 和 71 个 Prefill GEMM 用例。蓝色列由脚本直接回填 CUTLASS 数据；黄色和橙色列分别用于人工录入 HGEMM（自研）及 HGEMM（CUDA）结果；绿色列自动计算两者相对 CUTLASS 的加速比。
